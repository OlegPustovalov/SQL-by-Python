...
def add_phone(cursor,p_id_,phone_,client_id_):
    cursor.execute("""
    INSERT INTO phone_table(p_id,phone,client_id_)
        VALUES
        (%s, %s, %s);
...
def change_client(cursor, client_id_, first_name_, last_name_, email_):
    if first_name_ != '':
        ...
        set  first_name = %s,
        where client_id = %s;
        """,(first_name_, client_id_,)) 
    if last_name_ != '':
        cursor.execute(""" 
        update  client_table 
        set  last_name = %s,   
        where client_id = %s;
        """,(last_name_,client_id_,))
    ...
    if first_name_ != '':
        ...
        """,(email_,client_id_,)) 
    conn.commit()
    print('Запрос на изменение данных клиента выполнен')
    return
...
def find_client(cursor, client_id_,first_name_,last_name_,email_):
    print(f'Найдено с client_id {client_id_}')
    cursor.execute("""
    SELECT FROM client_table WHERE client_id=%s;
    """, (client_id_))
    conn.commit()
    cursor.execute("""
    SELECT FROM client_table WHERE first_name=%s and last_name=%s;
    ...
    """, (email_))
    conn.commit()
    print(f'Запрос поиска клиента выполнен')
    return
    ...
    with conn.cursor() as cur:
        ...
        while (n < 9) and (n > 0):
            ...
            if n == 7:
                ...
                last_name_ = input("Введите фамилию клиента -> ")
                email_ = input ("Введите эл. почту клиента -> ")
                find_client(cur,client_id_,first_name_,last_name_,email_)
